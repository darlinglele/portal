...
class Subscriber():
	...
	def retrieval(self,url):		
		try:
			...
			for entry in feeder.entries:			
				doc = {'site_url': url,'site_title': unicode(site_title)}						
	...
	def subscribe(self):		
		urls = [url.strip() for url in open('sites.txt','r')]		
		requests = threadpool.makeRequests(self.retrieval,urls, None) 
		for req in requests:
			self.pool.putRequest(req)
		self.pool.wait()

if __name__ == '__main__':
	subscriber = Subscriber(pool_size=10)
	print 'waitting...'
	while True:
